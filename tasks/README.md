...
| job_collector.bat | 04:00 | Mon to Fri | collect_premarket.py, runs to the 10:30 stop time on the 03:55 pool, rereading the watchlist from 07:20 and resubscribing when a new pool appears. A watchlist that is not today's is not subscribed to: since 2026-09-02 the run WAITS for today's file until the 07:20 handover, checking on the handover's own cadence, and refuses and exits non zero only if nothing has arrived by then, rather than subscribing to the context tickers alone and looking healthy while it does. Until 2026-09-02 it refused at once. The wait is asked for with `--wait-for-watchlist`, which this .bat passes and a hand run does not, so nothing run by hand can sit in it. It takes one optional argument, `stale-watchlist-ok`, and only monitor_jobs passes it: past the last pass that could rerun discover inside the window, CRITERIA decides possibly wrong names beat no tape, and the flag is how that branch says it knows. See CRITERIA.md [Monitor], the stale watchlist note |
| job_morning_chain.bat | 08:45 | Mon to Fri | scan.py, analyst.py, render_report.py, verify_morning.py, deliver.py, desk/render.py, ops/publish.py, stopping on the first failure. verify_morning.py is the exception: it prints the gate table for a human and never stops the chain, because the gate is enforced by deliver.py. A failure still redraws the desk and publishes it, each under its own "after failure" marker. publish.py uploads site/ to Cloudflare Pages and nothing else, refuses on an oversized file, a credential, a local path or inline data it cannot parse, skips cleanly without the Cloudflare values or while data\PUBLISH_HELD exists, and never fails the chain; its finished line is the watchdog's finish marker for this job |
| job_nightly.bat | 22:15 | Mon to Fri | the full firing of one task with three triggers; the .bat takes its mode from the clock, and a hand run may pass `full`, `catchup` or `universe` instead. THIRTEEN steps, in this order, and the list is closed: market_today.py as the trading day guard, market_today.py --refresh to renew the cached exchange calendar so the 08:45 chain never has to fetch it, backup_evidence.py to copy the six artifacts that cannot be rebuilt (the collector's capture, its stats and subscriptions sidecars, the packet, and the report in markdown and HTML) plus a dated snapshot of the quantifier flag log to somewhere outside the tree, backfill_premarket.py, fill_outcomes.py, true_volume.py to measure what premarket volume actually was from Alpaca's full SIP tape, paper_ledger.py to book the one written [Paper] rule against the levels that step just measured, pool_recall.py to measure what the morning's pool missed against every universe name that actually gapped, desk/compact.py to freeze each session's payload to runs/<date>/desk.json.gz, prune_data.py which is the only scheduled step in this project that DELETES anything and deletes only what its whitelist names, weekly_page.py, then desk/render.py so a broken morning still reaches a screen that evening, then ops/publish.py to upload site/ to Cloudflare Pages once both pages are written (held by data\PUBLISH_HELD until the owner lifts it; see ops/publish.py). [amended 2026-09-11: TWELVE became THIRTEEN when publish joined after the desk] [corrected 2026-09-04: named build_archive.py, which was retired that morning; the desk took its filename and its place at the end of both chains] [corrected 2026-09-04, again: said ELEVEN over a list of twelve and left desk/compact.py out of the list itself while a note beside it described where the step runs. That is the third time this row has been short by a step, so claim_the_nightly_list_names_every_step_it_runs now reads the .bat and refuses a module the row does not name] The refresh never fails the chain: a stale calendar is survivable, a failed refresh leaves yesterday's holiday list in place, and the morning records that it is stale. [corrected 2026-09-02: said TEN and omitted paper_ledger.py, which the .bat has run after true_volume.py since 2026-08-29, and said the backup copies two artifacts where _ARTIFACTS holds six] [corrected 2026-08-22: this listed five of the steps and named neither the backup nor the deletion, which are the two a reader most needs to know are happening] [corrected 2026-08-24: said NINE over a list of ten, which is what the .bat actually invokes; the trading day guard was being counted as free] |
| job_nightly.bat (the same task, its 07:00 trigger) | 07:00 | Mon to Fri | the catch-up firing, which the .bat recognises from a weekday clock before noon and which runs the vendor lag half only: the trading day guard, the calendar refresh, backup_evidence.py, backfill_premarket.py and fill_outcomes.py, then stops. The eight it skips are true_volume.py, paper_ledger.py, pool_recall.py, desk/compact.py, prune_data.py, weekly_page.py, desk/render.py and ops/publish.py. The vendor usually publishes intraday overnight, so this fills yesterday via the catch-up sweep and finishes the volume verification before the new morning's collection is trusted. pool_recall.py and the desk are skipped, because pool_recall measures the session it is invoked ON: until 2026-08-20 this firing asked for a session that had not opened and wrote gapped 0, addressable 0, recall 0.0 over the real measurement the 22:15 pass had taken |
| job_ladder.bat | 09:30, repeating every 2 min for 1 hour | Mon to Fri | market_today.py as the trading day guard, then ladder.py, then desk/render.py. The live ladder: where every name published at 08:45 stands against its own entry, right now. It is the only screen on the desk that moves, and it exists because 77 percent of the entries that ever trigger do so within five minutes of the open and the median time to trigger is ZERO, so the decision this desk supports is made between 09:30 and about 09:47 while the desk used to be silent from the collector's stop until the 12:00 pass. No vendor call: it reads the frozen packet for the levels and the collector's own file for the tape, which is why [Ladder] close_time and [Collector] stop_time are the same clock and must move together. It recomputes no level. Thirty firings an hour, so the watchdog deliberately gives it no due time and never reruns it: a missed firing is two minutes of a moving screen and the next one fixes it, while a relaunch would race the firing already due. A whole hour rather than to 10:15, because Task Scheduler's repetition duration is expressed in whole hours |
| job_midday.bat | 12:00 | Mon to Fri | scan_midday.py then render_midday.py. Grades every live picks row against the levels the morning published, and sweeps the whole universe for names that moved today and were never named at 08:45. No model runs: the report is rendered from the packet, because midday asks closed questions. The hour is chosen, not convenient: us-quote-delayed's REGULAR hours behaviour is what was measured and its premarket behaviour is untested. It costs about 2,900 credits a session, almost all of it the per symbol universe sweep, and the preflight refuses rather than truncating because half a universe is not a market wide scan. Then desk/render.py and ops/publish.py, so the published desk carries the Midday screen; neither fails the job. See CRITERIA.md [Midday] |
