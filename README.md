...
| 04:00 to 10:30 | collector | Websocket trades to one minute bars on disk, one file per day. Starts on the provisional pool the 03:55 discover wrote and moves onto the 07:15 pool at 07:20, keeping the tape of every name on both. See the two phase note in CRITERIA |
...
**Similar** `#/session/<date>/precedent`. What happened the last time a name
...
- **The watchdog usually acts first.** `src/ops/monitor_jobs.py` reruns anything
  ...
  again. Since the collector rereads the watchlist until its 10:30 stop time,
